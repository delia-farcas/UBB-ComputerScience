...
class BookService:
    def __init__(self, book_repo = BookRepository(), rental_repo = RentalRepository(), undo_service = UndoService(), validator = ValidatorBook()):
    ...
    def add_book(self, book_id: int, title: str, author: str):
        ...
        undo = FunctionCall(self._book_repo.remove, book_id)
        redo = FunctionCall(self._book_repo.add, book)
    ...
    def remove_book(self, book_id: int):
        ...
        # Record the undo/redo operation
        undo_book = FunctionCall(self._book_repo.add, book)
        redo_book = FunctionCall(self._book_repo.remove, book_id)
        ...
        try:
            ...
            # Create a cascaded operation for book and rentals
            operation = CascadedOperation(
                Operation(undo_book, redo_book),
                Operation(undo_rentals, redo_rentals)
            )
            self._undo_service.record(operation)
        except TypeError:
            operation = Operation(undo_book, redo_book)
            self._undo_service.record(operation)
        return book
    ...
    def update_book(self, book_id: int, new_title: str, new_author: str):
        ...
        :raises ValueError: If the book does not exist.
        """
        old_book = self._book_repo.find_by_id(book_id)
        ...
        # Record the undo/redo operation
        undo = FunctionCall(self._book_repo.update, old_book)
        redo = FunctionCall(self._book_repo.update, updated_book)
        operation = Operation(undo, redo)
        self._undo_service.record(operation)
