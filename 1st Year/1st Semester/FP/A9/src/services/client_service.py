...
class ClientService:
    def __init__(self, repository = ClientRepository(), rental_repo = RentalRepository(), undo_service = UndoService(), validator = ClientValidator()):
    ...
    def add_client(self, client_id: int, client_name: str):
        ...
        undo = FunctionCall(self._client_repo.remove, client_id)
        redo = FunctionCall(self._client_repo.add, client)
    ...
    def remove_client(self, client_id: int):
        ...
        # Record the undo/redo operation
        undo_client = FunctionCall(self._client_repo.add, client)
        redo_client = FunctionCall(self._client_repo.remove, client_id)

        try:
            undo_rentals = FunctionCall(self._rental_repo.add, *rentals)
            redo_rentals = FunctionCall(self._rental_repo.remove, *[rental.get_id for rental in rentals])
            # Create a cascaded operation for client and rentals
            operation = CascadedOperation(
                Operation(undo_client, redo_client),
                Operation(undo_rentals, redo_rentals)
            )
            self._undo_service.record(operation)
        except TypeError:
            operation = Operation(undo_client, redo_client)
            self._undo_service.record(operation)
        return client
    ...
    def update_client(self, client_id: int, new_name: str):
        ...
        """
        old_client = self._client_repo.find_by_id(client_id)
        ...
        # Record the undo/redo operation
        undo = FunctionCall(self._client_repo.update, old_client)
        redo = FunctionCall(self._client_repo.update, updated_client)
        operation = Operation(undo, redo)
        self._undo_service.record(operation)
