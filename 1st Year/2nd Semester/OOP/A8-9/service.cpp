...
void DogService::addDogWithId(const Dog& dog) {
    try {
...
Dog DogService::getDogById(int id) const {
    const auto& dogs = repository.getAllDogs();
    for (const auto& dog : dogs) {
        if (dog.getId() == id) {
            return dog;
        }
    }
    throw RepositoryException("Dog with ID " + std::to_string(id) + " not found");
}
